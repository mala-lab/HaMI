...
class EntailmentLLM(BaseEntailment):
    ...
    def check_implication(self, text1, text2, example=None):
        ...
        print(f"{self.name} prediction: {response}")

        binary_response = response.lower()[:50]
        if ('contradiction' in binary_response )or ('does not entail' in binary_response) or ('doesn\'t entail' in binary_response):
            # print('Contradiction')
            return 0
        elif ('entailment' in binary_response) or ('entail' in binary_response):
            # print('Entailment')
            return 2
...
class EntailmentLlama(EntailmentLLM):
    ...
    def predict(self, prompt, temperature):
        predicted_answer, _, _, _, _, _ = self.model.predict(prompt, temperature)
